...
export default function MacAppShell({ children, variant = 'store' }: MacAppShellProps) {
  ...
  return <MacShellProvider variant={variant}><div className="mac-workspace relative min-h-screen px-0 py-0 text-foreground sm:px-4 sm:py-4 lg:px-8 lg:py-8"><PointerGlow />
    <MacWindowIntro><div className="mac-window mx-auto flex min-h-screen max-w-[1480px] overflow-hidden sm:min-h-[calc(100vh-2rem)] sm:rounded-[1.25rem] sm:border sm:border-white/[0.08] sm:shadow-[0_32px_100px_hsl(160_60%_4%/.55)] lg:min-h-[calc(100vh-4rem)]">
      {!isAdmin && <aside className="mac-sidebar hidden w-[220px] shrink-0 flex-col border-r border-white/[0.07] px-3 py-4 md:flex" aria-label={es ? 'Navegación de tienda' : 'Store navigation'}>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="mac-titlebar flex h-14 shrink-0 items-center gap-3 border-b border-white/[0.07] px-4 sm:px-5" aria-label={isAdmin ? 'APERFY Admin Console' : 'APERFY Store'}>
          ...
          <div className="hidden flex-1 items-center justify-center md:flex"><span className="text-xs font-medium tracking-[.02em] text-muted-foreground">{isAdmin ? 'APERFY Console · operations and catalog' : (es ? 'APERFY Store · productos disponibles ahora' : 'APERFY Store · products available now')}</span></div>
