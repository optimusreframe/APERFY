...
export default function AdminMaterials() {
  ...
  const save = useMutation({ mutationFn: async (value: VariantPreset) => { const result = editId ? await supabase.from('materials').update(value).eq('id', editId) : await supabase.from('materials').insert(value); if (result.error) throw result.error; }, onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-variants'] }); setOpen(false); setEditId(null); setForm(empty); toast({ title: 'GUARDADO', description: 'PRESET DE VARIANTE ACTUALIZADO.' }); }, onError: (error: unknown) => toast({ title: 'ERROR', description: error instanceof Error ? error.message : 'Failed to save variant', variant: 'destructive' }) });
  ...
  const edit = (row: Material) => { setEditId(row.id); setForm({ name_en: row.name_en || '', name_es: row.name_es || '', description_en: row.description_en || '', description_es: row.description_es || '', is_active: row.is_active }); setOpen(true); };
