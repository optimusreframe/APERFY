...
const Email = ({ customerName, orderId }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your order #{orderId?.slice(0, 8).toUpperCase()} has been confirmed!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={logoSection}><Img src={LOGO_URL} alt={SITE_NAME} width="160" height="auto" style={{ margin: '0 auto' }} /></Section>
        <Section style={card}>
          <Heading style={h1}>Order Confirmed ✅</Heading>
          <Text style={text}>{customerName ? `Hi ${customerName},` : 'Hi,'} great news! Your order <strong style={{ color: '#fff' }}>#{orderId?.slice(0, 8).toUpperCase()}</strong> has been confirmed and is now being prepared.</Text>
          <Text style={text}>Our team is reviewing the details and will begin production soon. You'll receive another update when printing starts.</Text>
...
export const template = {
  component: Email,
  subject: (data: Record<string, unknown>) => `Order #${String(data.orderId || '').slice(0, 8).toUpperCase()} — Confirmed`,
