...
const Email = ({ customerName, orderId }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your order is now being 3D printed!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={logoSection}><Img src={LOGO_URL} alt={SITE_NAME} width="160" height="auto" style={{ margin: '0 auto' }} /></Section>
        <Section style={card}>
          <Heading style={h1}>Now Printing 🖨️</Heading>
          <Text style={text}>{customerName ? `Hi ${customerName},` : 'Hi,'} exciting news! Your order <strong style={{ color: '#fff' }}>#{orderId?.slice(0, 8).toUpperCase()}</strong> is now in production.</Text>
          <Text style={text}>Our 3D printers are working on your items right now. This process ensures every detail is perfect. We'll notify you once your order is ready for shipping.</Text>
...
export const template = {
  component: Email,
  subject: (data: Record<string, unknown>) => `Order #${String(data.orderId || '').slice(0, 8).toUpperCase()} — Now Printing`,
